...
import { InsertLoginAttempt, InsertUser, loginAttempts, users } from "../drizzle/schema";
...
export async function insertLoginAttempt(data: InsertLoginAttempt): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot log attempt: database not available");
    return;
  }
  try {
