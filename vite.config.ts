    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        if (reqBody && typeof reqBody === "object") {
          try { handlePayload(reqBody); } catch (e) { res.writeHead(400); res.end(JSON.stringify({ success: false, error: String(e) })); }
          return;
        }
        let body = "";
        req.on("data", (chunk) => { body += chunk.toString(); });
        req.on("end", () => {
          try { handlePayload(JSON.parse(body)); } catch (e) { res.writeHead(400); res.end(JSON.stringify({ success: false, error: String(e) })); }
        });
      });
    },
  };
